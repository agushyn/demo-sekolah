...
export default function EditHeroSlide({ slide }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'POST', // using POST with file upload for compatibility
        title: slide.title || '',
    ...
    const [imagePreview, setImagePreview] = useState(slide.image_url || null);
    ...
    return (
        <AdminLayout title="Edit Slide Hero">
            <Head title={`Edit Slide: ${slide.title} — Admin Portal`} />
            ...
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <Badge variant={data.is_active ? 'success' : 'neutral'} size="sm" dot>
                        ...
                        <div className="lg:col-span-8 space-y-6">
                            ...
                            >
                                <div className="space-y-4 pt-2">
                                    ...
                                    <label className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 hover:border-brand-400 rounded-2xl p-6 cursor-pointer bg-slate-50/50 hover:bg-brand-50/20 transition-all">
                                        <div className="w-10 h-10 rounded-2xl bg-white border border-slate-200 shadow-xs flex items-center justify-center text-slate-500 mb-2">
                                            <Upload className="w-5 h-5 text-brand-600" />
                                        </div>
                                        <p className="text-xs font-bold text-slate-800">
                                            {imagePreview ? 'Ganti Foto Background' : 'Klik untuk unggah foto'}
                                        </p>
                                        <p className="text-[11px] text-slate-400 mt-0.5">
                                            PNG, JPG, WebP, SVG hingga 5MB
                                        </p>
                                        <input
                                            type="file"
                                            accept="image/*"
